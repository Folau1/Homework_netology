...
Теперь обращения к порту `80` контейнера перенаправлялись на порт `81`, на котором работал Nginx. После проверки командой curl, возвращал стандартную HTML страничку.
